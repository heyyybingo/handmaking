...
**布局**：
- 主区域：全宽 Ant Design Table，展示作品缩略图、标题、分类、状态、统计数据
- 顶部操作栏：状态筛选下拉框、批量操作按钮（发布/下架/删除）、新建作品按钮
- 点击表格行或预览图标 → 弹出可拖动悬浮预览面板（`react-draggable`）

**悬浮预览面板（智能悬浮预览增强版）**：
- 标题栏可拖动（`.drag-handle`），带关闭按钮
- 默认位置：视口右中区域
- 预览模式（~400px宽）：展示小程序详情页视角的完整作品预览
- 编辑模式（~800px宽）：左侧实时预览 + 右侧富文本编辑表单

#### 作品编辑

**决策**：富文本编辑器 + 实时小程序预览

**编辑器**：WangEditor（`@wangeditor/editor`）

**理由**：
- 作品描述需要富文本格式（段落、加粗、列表等），与小程序 `rich-text` 组件渲染保持一致
- WangEditor 中文生态成熟，输出标准 HTML，小程序端可直接用 `rich-text` 渲染
- 轻量（~150KB），工具栏可定制，支持图片插入

**替代方案**：
- ~Markdown 编辑器~：非技术人员不友好，且小程序端需额外转换
- ~Quill.js~：功能类似但中文文档和生态不如 WangEditor
- ~TinyMCE~：过重，企业级功能本项目不需要

**编辑模式布局**：
- 左侧（flex: 1）：实时预览面板，即时展示编辑效果（与小程序详情页结构一致）
- 右侧（flex: 1）：编辑表单
  - 标题：Input（最长50字）
  - 描述：WangEditor 富文本编辑器，工具栏含加粗/斜体/下划线/标题/颜色/列表
  - 分类：Select 下拉选择
  - 标签：Select mode="tags" 自由输入
  - 状态：Select（草稿/已发布/已归档）
  - 排序：InputNumber
  - 图片：Upload picture-card（批量选择、拖拽排序、预签名直传）
  - 底部：保存/取消按钮

#### 小程序预览模拟器

**决策**：管理后台预览面板复刻小程序详情页结构

**理由**：
- 管理员需要看到用户视角的作品展示效果（WYSIWYG）
- 描述富文本渲染与管理后台编辑内容一致
- 减少"编辑完 → 去小程序看效果 → 回来调整"的往返操作

**预览面板布局（从上到下，对应小程序详情页）**：
1. 图片/视频轮播（Ant Design `Carousel` + `Image`，含指示器和计数）
2. 作品标题（18px 加粗）
3. 分割线
4. 作品描述（`dangerouslySetInnerHTML` 渲染富文本 HTML）
5. 分类标签（暖铜色 Tag）
6. 标签列表（小 Tag 横排）
7. 分割线
8. 统计行（👍点赞数 💬评论数 ❤️意向数）
